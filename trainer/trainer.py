...
from commons.utils import flatten_dict, tensorboard_gradient_magnitude, move_to_device, list_detach, concat_if_list, log, \
    tensorboard_class_accuracies, tensorboard_confusion_matrix, plot_confusion_matrix, plot_class_accuracies
...
class Trainer():
    def __init__(self, model, args, metrics: Dict[str, Callable], main_metric: str, device: torch.device,
                 ...
                 loss_func=ppiCrossEntropy, scheduler_step_per_batch: bool = True, run_dir='', weight=None, sampler=None):
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader):
        ...
        for epoch in range(self.start_epoch, self.args.num_epochs + 1):  # loop over the dataset multiple times
            self.epoch = epoch
            self.model.train()
            train_results, metrics, _, _ = self.predict(train_loader, optim=self.optim)
            self.model.eval()
            with torch.no_grad():
                val_results, metrics, _, _ = self.predict(val_loader)
                ...
                if val_score >= self.best_val_score and self.main_metric_goal == 'max' or val_score <= self.best_val_score and self.main_metric_goal == 'min':
                    ...
                    tensorboard_class_accuracies(train_results, val_results, self.writer, self.args, epoch + 1)
                    tensorboard_confusion_matrix(train_results, val_results, self.writer, self.args, epoch + 1)
                ...
                log('Epochs with no improvement: [', epochs_no_improve, '] and the best  ', self.main_metric,
                    ' was in Epoch No# ', epoch - epochs_no_improve)
    ...
    def forward_pass(self, batch):
        ...
        metadata = batch[-1]
        mask1 = torch.arange(metadata['length1'].max())[None, :] < metadata['length1'][:,
                                                                         None]  # [batchsize, seq_len]
        mask2 = torch.arange(metadata['length2'].max())[None, :] < metadata['length2'][:,
                                                                         None]  # [batchsize, seq_len]
        mask1 = move_to_device(mask1, self.device)                                                              
        mask2 = move_to_device(mask2, self.device)
        predictions = self.model(batch[0], batch[1], mask1, mask2)  # foward the rest of the batch to the model
    ...
    def predict(self, data_loader: DataLoader, optim: torch.optim.Optimizer = None, return_pred=False):
        ...
        for i, batch in enumerate(data_loader):
            ...
            *batch, = move_to_device(list(batch), self.device)
            ...
            pred_index = torch.max(predictions[..., :2], dim=1)[1]  # get indices of the highest value for loc
            results.append(torch.stack((pred_index, targets, ), dim=1).detach().cpu().numpy())
            with torch.no_grad():
                ...
                if self.optim_steps % self.args.log_iterations == 0 and optim != None:
                    ...
                    log('[Epoch %d; Iter %5d/%5d] %s: loss: %.7f Accuracy: %.7f lr: %.7f' % (
                        self.epoch, i + 1, len(data_loader), 'train', loss.item(), metrics["accuracy"], self.optim.param_groups[0]['lr']))
        ...
        if optim != None:
            return np.concatenate(results), total_metrics, None, None
        if optim == None:
            ...
            if return_pred:
                return np.concatenate(results), total_metrics, list_detach(epoch_predictions), list_detach(epoch_targets)
            else:
                return np.concatenate(results), total_metrics, None, None
    ...
    def evaluate_metrics(self, predictions, targets, batch=None, val=False) -> Dict[str, float]:
        ...
        for key, metric in self.metrics.items():
            if not hasattr(metric, 'val_only') or val:
                ...
                metrics[key] = metric(concat_if_list(predictions), concat_if_list(targets))
        return metrics
    ...
    def evaluation(self, data_loader: DataLoader, data_split: str = '', return_pred=False):
        ...
        results_, metrics, predictions, targets = self.predict(data_loader, return_pred=return_pred)

        plot_class_accuracies(results_,os.path.join(run_dir, 'class_accuracies_' + data_split + '.png'), self.args)
        plot_confusion_matrix(results_,os.path.join(run_dir, 'conf_matrix_' + data_split + '.png'))
